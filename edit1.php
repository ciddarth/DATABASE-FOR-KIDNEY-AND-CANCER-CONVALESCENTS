...
if(isset($_POST['search']))
{
	...
	$search_Query="SELECT * FROM once WHERE id =$data[0]";
...
{
	$data = getPosts();
	$insert_Query = "INSERT INTO `once`(`fname`, `lname`, `age`) VALUES ('$data[1]','$data[2]','$data[3]')";
	try{
...
{
	$data = getPosts();
	$delete_Query = "DELETE FROM `once` WHERE `id` = $data[0]";
	try{
...
{
	$data = getPosts();
	$update_Query = "UPDATE `once` SET `fname`='$data[1]',`lname`='$data[2]',`age`='$data[3]' WHERE =$data[0]";
	try{
...
<body style="background-color:#93927b;">
	<form action="edit1.php" method="post"><center>
