...
if(isset($_POST['search']))
{
	...
	$search_Query="SELECT * FROM entry1 WHERE id =$data[0]";
...
{
	$data = getPosts();
	$insert_Query = "INSERT INTO `entry1`(`fname`, `lname`, `age`) VALUES ('$data[1]','$data[2]','$data[3]')";
	try{
...
{
	$data = getPosts();
	$delete_Query = "DELETE FROM `entry1` WHERE `id` = $data[0]";
	try{
...
{
	$data = getPosts();
	$update_Query = "UPDATE `entry1` SET `fname`='$data[1]',`lname`='$data[2]',`age`='$data[3]' WHERE =$data[0]";
	try{
...
<body style="background-color:#41D5A6;">
	<form action="edit.php" method="post"><center>
